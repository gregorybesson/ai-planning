...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # let's initialize the a_levels set
        self.a_levels.append(set())
        # and check the eligible actions
        for action in self.all_actions:
            a_node = PgNode_a(action)
            #if states set in the level is a superset of action preconditions
            if self.s_levels[level].issuperset(a_node.prenodes):
                # then the action is eligible
                self.a_levels[level].add(a_node)
                # And I create the links between states and actions accordingly
    ...
    def add_literal_level(self, level):
        ...
        # let's initialize the s_levels set
        self.s_levels.append(set())
        # and parse each action node from the previous level
        for a_node in self.a_levels[level - 1]:
            # from which we parse the literals
            for s_node in a_node.effnodes:
                # and add them to the s level, and create relations between a_node and s_node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # We have an inconsistency only IF NOT
        # one effect_add from node_a1 is not in effect_rem from node_a2
        # AND 
        # one effect_add from node_a2 is not in effect_rem from node_a1
        
        add_1 = set(node_a1.action.effect_add)
        rem_1 = set(node_a1.action.effect_rem)
        add_2 = set(node_a2.action.effect_add)
        rem_2 = set(node_a2.action.effect_rem)

        consistent = add_1.isdisjoint(rem_2) and add_2.isdisjoint(rem_1)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        add_2 = set(node_a2.action.effect_add)
        rem_2 = set(node_a2.action.effect_rem)
        pos_1 = set(node_a1.action.precond_pos)
        neg_1 = set(node_a1.action.precond_neg)
        pos_2 = set(node_a2.action.precond_pos)
        neg_2 = set(node_a2.action.precond_neg)

        no_interference = add_1.isdisjoint(neg_2) and add_2.isdisjoint(neg_1) and rem_1.isdisjoint(pos_2) and rem_2.isdisjoint(pos_1)

        return not(no_interference)
